...
start_expo() {
  ...
  # Use static export if available, avoids Metro bundler hanging issues
  if [ -d "$ROOT_DIR/client/web-static" ] && [ -f "$ROOT_DIR/client/web-static/index.html" ]; then
    echo "Using pre-exported static files for web preview"
    ( nohup python3 "$ROOT_DIR/.cozeproj/scripts/serve-static.py" "$EXPO_PORT" "$ROOT_DIR/client/web-static" 2>&1 | pipe_to_log "CLIENT" "$LOG_CLIENT_FILE" ) &
  else
    echo "Static files not found, falling back to Expo bundler"
