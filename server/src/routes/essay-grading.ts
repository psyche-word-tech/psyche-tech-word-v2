...
【二档（6-10）】情节不完整/推进不足（L2），或题目确实要求两段但只认真写了一段、第一段写得好（情节完整、语言尚可）可升至二档上沿；衔接一般，错误很多。
【一档（1-5）】情节碎片、基本无推进（L1），衔接生硬，错误极多。
【0分】离题或没有写任何自主内容。

【硬性贯穿规则】
1. 第一权重是情节/衔接质量：先按情节完整度（L0-L5）判断大致档位，再校验语言/篇幅；切勿仅凭词数或错误数量定档、也不得仅凭词汇堆砌升档。
2. 二段没写≠一定一档，但升档有严格前提：只有第一段【完整展开了一个冲突场景】且【有明确的后续走向/收势】（第一段本身基本构成自洽冲突单元，可看出思路已组织好只是字数不济未完卷），且语言尚可，才可升到二档（6-10），通常落二档中下（6-8）；若第一段只是零星对话、几句带过、情节碎片化、没有完整展开任何冲突场景，则【一律一档（1-5）】。
...
【铁律：提示句不算考生所写】题目给的第二段段首提示句（如 "Later that night, I received their apology messages."）不属于考生作文内容。凡第二段只有这句提示句、其后没有任何自主书写内容，一律按【二段未写】判定；不得因该句被 OCR 识别进文本而把第二段当作"有内容"/"完整"。二段未写时：若第一段完整展开冲突场景且语言尚可→最高二档（通常6-8）；若第一段仍属情节碎片/几句话带过→一档（1-5），与第2条一致。
