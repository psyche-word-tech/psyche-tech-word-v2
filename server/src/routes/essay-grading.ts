...
router.post('/grade', optionalAuthMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (isContinuation && gradingResult.comments) {
      ...
      // ——【13分起评】硬规定兜底（读后续写）：两段都完整展开 + 情节主线合理自洽 → 总分从 13 起评。
      // 评语若承认"两段完整/两段都写/两段均完成/两段都认真写/情节合理/情节自洽/两段都推进/完整续写"，但 total_score 低于 13，
      // 视为模型起评过低，强制抬到 13（仅当评语同时出现明显情节硬伤标志才不抬）。三档 min=11 及以上适用。
      const completeTwo = /两段(都)?(?:完整|写完|写完现)|两段都写|两段均完成|两段都认真写|两个段落都(?:完整|完成)|完整续写|两段都推进|两段(?:均|都)(?:完整|完成|写完)/.test(c) ||
        /情节(?:上|上)?(?:是)?完整的|情节完整|完整展开|续写(?:是)?(?:完整|完整)|情节(?:上)?(?:是)?连贯|完整地(?:推进|展开)|情节(?:上)?(?:是)?(?:合理|自洽)|符合(?:读后续写|题目|写作)?(?:的)?要求/.test(c);
      const plotOk = /情节(?:主线)?(?:合理|自洽|成立|完整|连贯|顺畅|符合逻辑|有效推进)|合理自洽|逻辑自洽|冲突|升华|解决|感悟|和解|情绪(?:宣泄|表达)/.test(c);
      const plotFlaw = /情节(?:混乱|断裂|硬伤|跑题|严重偏离|戛然而止)|逻辑断裂|明显硬伤|跑题|严重偏离|混乱断裂/.test(c);
      if (isContinuation && (tierInfo ? tierInfo.min >= 11 : /三档|3\s*档|11-15|11[~-]15/.test(c)) && completeTwo && plotOk && !plotFlaw) {
        if (gradingResult.total_score < 13) {
...
本作文是英语读后续写，满分${maxScore}分（25分制，六档：五档21-25、四档16-20、三档11-15、二档6-10、一档1-5、0分）。**不要**按"内容/语言/结构/书写"四维直接估分相加，也不要按自定义维度给分。必须按下面"情节完整度第一权重、从高到低判档"的规则判档，杜绝跳档套分。

【核心原则】情节/衔接质量是第一权重，贯穿所有档位；词数/错误/词汇只作档内二次微调；字数必须与原文情节高度相关（先决条件）。判档主依据是续写情节的完整度与衔接质量，而非词数或错误数量。

【铁律 #1 —— 错误绝不降档（最高优先级，违反整题判错）】语言表达（拼写/语法/词汇/错误**数量与严重程度**）**绝对不允许**用作降档依据，只能在同一档内微调分数（错误多→贴档内下沿，错误少→贴档内上沿），**严禁因错误多/错得低级就把整篇压到更低档**。判档唯一依据是情节/衔接质量。具体：
- **只要情节完整且给出结局/升华**（冲突→推进→解决/感悟，两段都算数，第二段只要扣住段首句展开即可）——即使错误极其密集、极其基础，**也锁死在三档（11-15）及以上，绝不允许因语言差降到二档**；其中**若两段都完整且情节主线合理，总分起评不得低于 13**（见 L3 硬规定），语言差最多贴 13 而不降到 11-12，只有两段虽完整但情节有明显硬伤才能落 11-12。
...
【第二步·按情节完整度定主航向】（第一权重，决定落哪个档区）
把续写情节按完成度从高到低分级，作为档位主依据：
- L5 情节完整且收束有力：连贯完成"冲突→推进→解决→情感升华/感悟/呼应原文"，衔接自然、逻辑严密，且叠加多样化高中词或多样化语法 -> 方可进四档上沿及五档（16-25）。
- L4 情节完整、结局明确，并有【明确的情感升华/顿悟/成长感悟】——结尾能看到考生总结道理、点出情感走向、或呼应原文意象（如对友谊/原谅的领悟），衔接自然：可进三档上沿到四档（14-20）。凡第二段结尾有明白的成长领悟、道理升华、情感收束，应主动判到 L4，落分靠档内上沿，不要保守压低。
- L3 情节完整（冲突→有明确结局/和解），逻辑自洽、衔接基本顺畅：进三档（11-15）。
  - 【硬规定：13分起评】凡**两段都完整展开、且情节主线合理自洽**（冲突→推进→结局/和解顺序成立、不是敷衍的空洞两段）——总分**从 13 分起评**（三档中上），语言/错误/词数只能在 13 分之上做档内微调（取 13-15），**绝不允许起评低于 13**。**语言差（错误密集、基础拼写/词汇错误、中式表达、词性误用、影响阅读流畅度）绝不构成"降到 11-12"的理由**——这些只能让 13 分内的分偏低（13），不能掉出 13。仅当**情节本身**有硬伤（逻辑断裂/严重偏离段首句/戛然而止无推进/两段空洞敷衍）时，才允许落到 11-12 的贴下沿水平。
- L2 情节不完整：未写结局，但已完整展开一个冲突场景（如面对面对峙/摊牌并有大段推进行为，第一段本身构成相对完整的冲突单元）：三档下沿到二档（6-12）。
- L1 情节碎片：仅一两个短暂场景、几句话带过、没有完整展开任何冲突场景、无推进无结局——即使第一段写了字，也只算情节碎片：一档（1-5）。
- L0 离题/无自主内容：0 分。

【判档（主依据=情节完整度，从高到低）】
【五档（21-25）】情节完整且收束有力（L5），衔接非常顺畅、逻辑严密（多种衔接：因果、让步等），并运用多样化高中词与多样化语法，错误极少。
【四档（16-20）】情节完整、结局明确、见情感收束（L4），衔接自然顺畅，词汇多样、有高中词，语法较多样，错误不多（可参考不超过10处）。
【三档（11-15）】情节完整、逻辑自洽、结局明确（L3），衔接基本顺畅，词汇与语法较基础、高中词少，错误较多（可参考10-15处）。
【二档（6-10）】情节不完整/推进不足（L2），或题目确实要求两段但只认真写了一段、第一段写得好（情节完整**且语言洁净：错误约1-4个、无明显句法/句式问题**）可升至二档（通常6-8）；衔接一般，**错误偏多但有句法过关**。
【一档（1-5）】情节碎片、基本无推进（L1），衔接生硬，错误极多。
【0分】离题或没有写任何自主内容。

【硬性贯穿规则】
1. 第一权重是情节/衔接质量：先按情节完整度（L0-L5）判断大致档位，再校验语言/篇幅；切勿仅凭词数或错误数量定档、也不得仅凭词汇堆砌升档。
2. 二段没写≠一定一档，但升档有严格的双重前提，**缺一不可**：只有第一段【完整展开了一个冲突场景】且【有明确的后续走向/收势】（第一段本身基本构成自洽冲突单元，可看出思路已组织好只是字数不济未完卷）**且语言几乎无明显错误**——**错误总数很少（约 1-4 个）且没有任何句法/句式问题**（拼写/单词级小错可以，但句子结构残缺、语序混乱、杂糅等句法级问题即视为不过关），才可升到二档（6-10），通常落二档下沿（6-8）。**若第一段虽情节完整但错误偏多（超过约4个）或存在句法/句式问题，则不给二档，压到一档上沿附近（约5分上下一分，即4-6）**；若第一段只是零星对话、几句带过、情节碎片化、没有完整展开任何冲突场景，则【一律一档低段（1-4）】。这条的总原则：**二段未写时，"第一段写得好"必须同时满足情节完整+语言洁净，语言是硬门槛——错误多的不能仅凭情节撑到二档**。
3. 词数必须与原文情节高度相关：仅为凑数/大量复述原文不给分；情节相关度高且篇幅充足是升档加分，篇幅过短（不足150词的一半）或严重低于要求时一般难以进入四档以上。
4. 档内给分一律以 0.5 为步长：同一档内，词数足/错误少/高中词多→贴近档内上沿；词短/错误多→贴近下沿。绝不在档外给分。**【书写整洁奖励】在同一档内其他条件相近时，若卷面书写整洁清晰、字迹工整易读，可在该档内上浮 1~1.5 分（不跨档到上一档）；书写潦草/难以辨认则不上浮、偏向档内中下。判定整洁的物理判据：字迹工整规范易读、整体卷面清爽、绝大多数单词可辨认。常见忽略项（不构成「不整洁/潦草」）：个别单词连笔稍随意、个别划改/涂改（只要不影响辨认）、少量涂改痕迹、个别重写拼写。只有整体字迹潦草难辨、多处乱涂、或大面积难以辨认时才判「书写潦草/卷面一般」并取消奖励。判定时请结合原文图片辅助判断真实卷面，不可仅凭转录文本猜测卷面整洁度。
5. 只输出单一 total_score（不输出内容/语言/结构/书写等子维度分），total_score 必须落在所判档区间内。
6. 评语须写明：先从情节完整度判到哪一级（L几）、据此落入哪一档，再说明档内落到此分数的语言/篇幅理由；措辞与分数趋势一致（评语严厉则分数必须低，反之亦然）。
【铁律：提示句不算考生所写】题目给的第二段段首提示句（如 "Later that night, I received their apology messages."）不属于考生作文内容。凡第二段只有这句提示句、其后没有任何自主书写内容，一律按【二段未写】判定；不得因该句被 OCR 识别进文本而把第二段当作"有内容"/"完整"。二段未写时：仅当第一段完整展开冲突场景**且语言洁净（错误约1-4个、无句法/句式问题）**→最高二档（通常6-8）；若第一段情节完整但**错误偏多（超约4个）或存在句法/句式问题→不给二档，压到一档上沿附近（约5分上下，4-6）**；若第一段属情节碎片/几句话带过→一档低段（1-4）。与第2条一致。

【输出要求】tier 填实际锁定档（如 五档{min:21,max:25}、四档{min:16,max:20}、三档{min:11,max:15}、二档{min:6,max:10}、一档{min:1,max:5}、0分{min:0,max:0}），total_score 落在该档区间内且等于评语/分析中你能给出的档内评分，不拆四维。`
    : '';;
...
## 评分标准 / 档次 / 扣分规则（若提供，必须遵循）
${gradingStandard || '（未提供，按常见作文评分惯例先定档再估分）'}

【如何使用上面的标准——至关重要】
- 若给出了"档次/分档"描述（如 第一档13-15 / 第二档9-12 / …，或 一类文/二类文/三类文），先逐档对照作文判定它属于哪一档，**总分必须落在所定档的区间内**，并在 comments 里写明定档理由。
- 若给出了**明确的评分维度清单**（一组并列的维度即可，如 内容要点/词汇语法/篇章连贯，无论是否附分值；或 内容5分/语言5分/结构3分 这类"维度+分值"），必须按这些维度**逐项单独打分**：points 数组每个维度一条 {point:维度名, max:该维度满分, score:实得分, comment:理由}，且 total_score=各 point 的 score 之和。若标准未给各维度满分，则你自行按重要性把 max_score 分配到各维 max（各维 max 之和 = max_score），再在各 max 内估分。此时 scores 四维填 0。**仅当标准里没有任何自定义维度时才**用默认四维（内容/语言/结构/书写）并把 points 留空。
...
- 评语的定性措辞必须与所给档位/分数**严格匹配**：若评语明确写"大量错误、严重语法错误、严重影响理解"，则只在偏低档位（并按高考五档此处=第二档 4-6 及以下）给分；若给了中高分，评语只能表述为"有若干不足但基本完成任务、基本不影响理解"。禁止出现"评语严厉批评缺点的严重程度"与"中高分数"同时出现、互相矛盾。
- 【读后续写降档硬约束】若是续写/任务型作文：**只写了一段（缺第二段）时，晋级上限自动锁到二档（6-10）**，无论其他方面多好都不得进入三四五档；但档内仍按该档各项达成情况以 0.5 步长给分，不强制取档下限。任务完成度严重缺失时（情节未写完整、未衔接所给开头语/原文），评语必须写明"情节不完整/字数不足/段落未写满"。**绝不因"语言其实还行"或"少犯语法错"就把只写一段的续写抬到中高档**——段落任务是续写档位的第一硬指标。
