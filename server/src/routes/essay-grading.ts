...
router.post('/grade', optionalAuthMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (isContinuation && gradingResult.comments) {
      ...
      const completeTwo = /两段(都)?(?:完整|写完|写完现)|两段都写|两段均完成|两段都认真写|两个段落都(?:完整|完成)|完整续写|两段都推进/.test(c);
      const plotOk = /情节(?:主线)?(?:合理|自洽|成立|完整|连贯|顺畅|符合逻辑|有效推进)|合理自洽|逻辑自洽/.test(c);
...
- L3 情节完整（冲突→有明确结局/和解），逻辑自洽、衔接基本顺畅：进三档（11-15）。
  - 【硬规定：13分起评】凡**两段都完整展开、且情节主线合理自洽**（冲突→推进→结局/和解顺序成立、不是敷衍的空洞两段）——总分**从 13 分起评**（三档中上），语言/错误/词数只能在 13 分之上做档内微调（取 13-15），**绝不允许起评低于 13**。仅当两段虽完整但情节有明显硬伤（逻辑断裂/严重偏离段首句/戛然而止无推进）时，才允许落到 11-12 的贴下沿水平。
