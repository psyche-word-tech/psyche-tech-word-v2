...
【语言绝不跨档——铁律】语言（拼写/语法/词汇/错误数量）只能在**已由情节定下的档内**做上下微调，**绝对不允许**因为语言差就把档位本身往下踩。尤其：只要两段续写都完整展开、有明确结局（冲突→推进→解决/感悟），即使错误极多、行文破碎，**也至少判三档（11-15）下沿到二档（6-10）**，严禁判一档。判档只问"情节是否完整、是否推进、有无结局/升华"，语言问题留到判档之后在档内扣分。若你评语里已经承认"情节完整/起承转合都有"，那档位就绝不能是一档——评语与档位矛盾视为判错。
