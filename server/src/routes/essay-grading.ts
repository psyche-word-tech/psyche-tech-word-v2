...
- 【读后续写降档硬约束】若是续写/任务型作文且**任务完成度严重缺失**（只写一段、字数远低于要求、情节未写完整、未衔接所给开头语/原文），档位必须压到最低档区间（25 分满时≤第一/二档、15 分满时≤第一档），总分取该档下限，评语必须写明"情节不完整/字数不足/段落未写满"。**绝不因"语言其实还行"或"少犯语法错"就把任务完成度缺失的续写抬到中高档**——任务完成度是续写的第一硬指标。
