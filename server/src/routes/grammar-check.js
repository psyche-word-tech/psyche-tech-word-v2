...
// LanguageTool 错误信息翻译
function translateLTMessage(msg, ruleId) {
    const translations = {
    ...
    // 关键词匹配翻译
    if (msg.includes("Possible spelling mistake"))
        return "可能存在拼写错误";
    if (msg.includes("Did you mean"))
        return msg; // 保留建议
    if (msg.includes("agreement"))
        return "主谓不一致";
    if (msg.includes("tense"))
        return "时态错误";
    if (msg.includes("article"))
        return "冠词使用不当";
    if (msg.includes("preposition"))
        return "介词使用不当";
    if (msg.includes("comma"))
        return "逗号使用不当";
    if (msg.includes("capitalization"))
        return "大小写错误";
    if (msg.includes("wordiness"))
        return "表达冗长，建议简化";
    if (msg.includes("passive voice"))
        return "建议使用主动语态";
    if (msg.includes("Possible typo"))
        return "可能存在打字错误";
    // 默认返回原始信息（如果已经比较简短）
    return msg.length > 80 ? msg.substring(0, 80) + "..." : msg;
}
function generateCorrectedText(text, matches) {
    let corrected = text;
    // 从后向前替换，避免位置偏移
...
// ============ LLMClient（沙箱环境内置）============
async function callLLM(text, _language) {
    try {
...
// ============ OpenAI（需要配置 API Key）============
async function callOpenAI(text, _language) {
        ...
        const response = await globalThis.fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: 'You are a professional English grammar checker. Analyze the text for grammar, syntax, and style errors. Return ONLY a JSON object with this exact structure: {"isCorrect": boolean, "issues": [{"title": "brief Chinese title", "message": "detailed Chinese explanation", "replacements": ["suggestion1"]}], "correctedText": "corrected version"}.',
                    },
                    {
                        role: "user",
...
function ruleBasedGrammarCheck(text) {
    const issues = [];
    const doc = nlp(text);
    // 1. 句子首字母大写
...
// ============ 具体规则实现 ============
function checkCapitalization(doc, text, issues) {
...
function checkSubjectVerbAgreement(doc, issues) {
    const sentences = doc.sentences().json();
    for (const sent of sentences) {
...
function checkUncountableNouns(doc, issues) {
    const uncountable = [
...
function checkComparative(doc, issues) {
...
function checkThereTheirTheyre(text, issues) {
    const patterns = [
...
function checkPrepositionRedundancy(doc, issues) {
    const redundant = [
...
function checkWordiness(doc, issues) {
    const wordy = [
...
function checkDoublePunctuation(text, issues) {
    if (/,\s*,/.test(text)) {
        issues.push({
            title: "连续逗号",
...
function checkInconsistentCapitalization(text, issues) {
...
function checkArticleUsage(doc, issues) {
    const text = doc.text();
    // a + 元音音素
...
// 11. 间接疑问句语序
function checkIndirectQuestion(text, issues) {
    const patterns = [
...
// 12. 虚拟语气
function checkSubjunctive(text, issues) {
...
// 12.5 及物动词后代词应使用宾格
function checkObjectPronoun(doc, issues) {
...
// 13. 介词后动词应使用动名词
function checkPrepositionVerbForm(doc, issues) {
    const text = doc.text();
...
function checkComparativePronoun(text, issues) {
    const patterns = [
...
// 15. since/for + 时间段混淆
function checkSinceFor(text, issues) {
    const patterns = [
...
function checkSuggestInsistSubjunctive(text, issues) {
    const patterns = [
...
// 17. 反身代词与主语不匹配
function checkReflexivePronoun(doc, text, issues) {
    const patterns = [
...
function checkRelativeClause(doc, issues) {
    const text = doc.text();
...
// 19. 形容词/副词混淆
function checkAdjectiveAdverb(doc, issues) {
    const text = doc.text();
    const adjAdvPairs = {
        ...
        if (matches) {
            for (const match of matches) {
                ...
                if (senseVerbs.includes(verb))
                    continue;
                // 排除 be 动词
                if (verb === "be" || verb === "am" || verb === "is" || verb === "are" || verb === "was" || verb === "were")
                    continue;
                issues.push({
                    title: "形容词/副词混淆",
...
// 20. 时态一致性
function checkTenseConsistency(doc, issues) {
    const sentences = doc.sentences().json();
    for (const sent of sentences) {
        const sentDoc = nlp(sent.text);
        const verbs = sentDoc.verbs().json();
        if (verbs.length >= 2) {
            const tenses = verbs.map((v) => {
                const tags = v.tags || [];
...
function generateSimpleCorrection(text, issues) {
    let corrected = text;
    // 简单的替换逻辑（实际应基于位置精确替换）
    return corrected;
}
