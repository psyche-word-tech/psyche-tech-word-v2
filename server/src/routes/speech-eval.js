...
function convertToWavBuffer(inputBuffer) {
...
async function getBaiduAccessToken() {
    ...
    try {
        const res = await fetch(`https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=${API_KEY}&client_secret=${SECRET_KEY}`, { method: "POST" });
...
/** 调用百度语音识别 API */
async function baiduAsrRecognize(wavBuffer, token) {
...
router.post("/", upload.single("audio"), async (req, res) => {
    ...
    try {
        ...
        if (!transcription) {
            ...
            try {
                const testRes = await fetch(`https://aip.baidubce.com/oauth/2.0/token?grant_type=client_credentials&client_id=${process.env.BAIDU_ASR_API_KEY}&client_secret=${process.env.BAIDU_ASR_SECRET_KEY}`, { method: "POST" });
