...
router.get('/', async (req, res) => {
    try {
      ...
      `)
        ...
        // 格式化数据
        const words = data?.map((item) => item.words).filter(Boolean) || [];
        res.json(words);
    }
    catch (err) {
...
router.post('/move-mindmap', optionalAuthMiddleware, async (req, res) => {
    try {
        ...
        const otherTables = validTargets.filter((t) => t !== targetTable);
...
router.get('/mindmap-counts', optionalAuthMiddleware, async (req, res) => {
    try {
        const client = getSupabaseClient();
        const userId = req.userId || 1;
