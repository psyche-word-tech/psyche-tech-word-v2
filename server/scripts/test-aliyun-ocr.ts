...
async function testOCR() {
  ...
  // 使用示例图片 URL
  console.log('使用示例图片...');
  const response = await fetch('https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20241108/ctdzex/biaozhun.jpg');
  const buffer = await response.arrayBuffer();
  const base64 = Buffer.from(buffer).toString('base64');
  
  console.log('调用阿里云 OCR...');
  const result = await callAliyunOCR(base64);
