...
#### 读后续写评分已重写为「从高到低逐档比对」六档（用户最终确认版，替代旧"两步法"）
- **判档顺序**：把续写**依次与五档→四档→三档→二档→一档→0分标准逐一比对**，先试最高档，符合某档**全部**条件才入档，进不了再下移比低一档，禁止跳档、禁止先定低档再上调。档位完全由逐档比对的命中结果决定。
- **档位区间（满分25，0.5为步长）**：
  - 五档 21-25：字数≥140、情节高度相关、词汇多样（高中较难词汇占比高）、语法多样、与提示段衔接非常顺畅+逻辑严密（多种衔接：因果/让步等）、错误**极少**。
  - 四档 16-20：字数**≥131（含131-139）**、情节高度相关、词汇多样但高中词**占比不大**、语法多样、衔接一般+逻辑不太严密+连接词较少、错误**比较多（约>10处）**。
  - 三档 11-15：字数100-130、**两段都有内容**、情节高度相关、词汇有限且高中词占比很小、语法有限、衔接一般+逻辑不严密+连接词很少、错误**很多（约>15处）**。
  - 二档 6-10：**50<字数<100**、两段都有内容、情节高度相关、词汇有限基本义务教育词、语法有限、衔接一般+逻辑不严密+连接词很少、错误**很多（约>20处）**。
  - 一档 1-5：字数<50、情节与原文**有一定**相关性、词汇有限基本义务教育词、语法有限、衔接一般基本无句间连接词、错误很多。
  - 0分：内容与原文**完全无关**，或没写任何内容（原文提示句不算考生所写）。
- **边界确认（用户拍板）**：131-139 词落四档；**只写了一段晋级上限锁到二档**（缺"两段都有内容"硬资质，无论其他多好不得进三四五档）；错误数量**定性处理**（极少<比较多<很多，不硬绑数字，用于档内定分与跨档辅助）。
- **档内给分**：0.5 步长，落在该档区间内，越接近上限说明该档各项达成越好，越接近下沿说明欠缺越多；绝不在档外给分。
- **代码落点**：前端 `CONTINUATION_STANDARD` 常量（`client/screens/essay-grading/index.tsx` 顶部，随"英语读后续写"开关 switchContinuation 自动预填，满分自动 25）；后端 `essay-grading.ts` 里 `continuation` 分支的 prompt（`gradingStandard` 非空即按档位锁 total_score）。前端识别续写用 `isContinuationMode = isContinuation || /读后续写|续写|两步法|档位|五档/.test(gradingStandard)`，与后端自动识别（`/读后续写|续写|两步法|档位|1档|一档/`）保持一致。
- **实测**：写作二.jpg（约70-80词+两段+情态误用/词性误用/拼写连写等多处基础错误）→ **7/25 二档(6-10)**（old宽判11、then过紧0分均被根治）。改前端后须 `expo export --platform web`+复制到 `server/public` 才生效。
