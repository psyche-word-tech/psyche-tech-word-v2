...
- **【13分起评硬规定（用户2026-09指定）】**：读后续写只要**两段都完整展开、且情节主线合理自洽**（冲突→推进→结局/和解顺序成立、非敷衍空洞两段），总分**从 13 起评**（三档中上），语言/错误/词数只能在 13 之上做档内微调，**不得低于 13**；仅两段虽完整但情节有明显硬伤（逻辑断裂/严重偏离段首句/戛然而止无推进）才落 11-12 贴下沿。已写入：前端 CONTINUATION_STANDARD 的 L3 与铁律#1、后端 CONT prompt 的 L3 硬规定与铁律#1、以及后端 13分起评程序化兜底（评语含"两段完整/完整续写"+“情节合理/自洽”且无硬伤标志、total_score<13 时强制抬到 13）。
