...
#### 读后续写评分已细化为「两步法」精确五档（用户最终确认版，覆盖上面的通用续写标准）
- **第一步·情节与融洽（基本资质）**：要求两段、约150词（每段约80）、情节完整且与原文/各段开头语衔接合理。
- **情况一（硬锁档）**：字数<80 或 只写了一段 → **直接归 1/2 档**，不看融合。再按第二指标细分：词汇语法丰富准确 → 2档(6-10)；错误多 → 1档(0-5)。
- **情况二（进入 3/4/5 档）**：仅当两段齐全、字数达标、情节融合且衔接合理。细分：5档(21-25)=衔接合理+逻辑严密+词汇语法复杂化高+错误≤3-4；4档(16-20)=逻辑稍不严密+错误约8-12+但词汇语法多样化；3档(11-15)=衔接不太合理+逻辑不严+错误12以上+句间不连贯+用词简单。
- **判分执行**：先两步锁档区间，再在档内由模型自行细化（0.5 步长）；任务完成度不达标无论语言多好都**绝不进 3 档及以上**；评语须写明分档依据（走了哪种情况、字数/段落实况、错误数）。
- **代码落点**：前端 `CONTINUATION_STANDARD` 常量（`client/screens/essay-grading/index.tsx` 顶部，随"英语读后续写"快捷开关 switchContinuation 自动预填，满分自动 25）；后端 `essay-grading.ts` prompt 在 `gradingStandard` 非空时按档位锁 total_score。
- **实测**：写作二.jpg（一段+约70词+密集语病）→ 总分 **5/25 第一档**（content1.5/language1.5/structure1），评语明示"字数不足直接锁最低档"，由虚高的 11/25 根治为第一档。改前端 `CONTINUATION_STANDARD` 后须 `expo export --platform web` + 复制到 `server/public`（复用前文 expo bin 绝对路径命令）才随 bundle 生效。
