...
21. **手机浏览器注册页密码框软键盘"闪一下消失"无法输入（已修复/根因已定）**：症状为手机浏览器（华为鸿蒙/小米异常，iQOO 老版正常，电脑正常）直连 Railway 后，注册页点"设置密码"时软键盘闪一下消失、无法输入。**根因（playwright 移动端实测定案）**：新版手机浏览器默认 `interactive-widget=resizes-visual`——软键盘**悬浮弹出、布局视口保持原高（844）不变**，而 RN Web reset（`html/body/#root{height:100%}` + `body{overflow:hidden}`）把整页钉死在视口高、内容永不溢出，无任何可滚动区域；密码框（position top≈441-527）在物理屏幕上被键盘盖住，RN 容器又不感知软键盘、仍 844 高，浏览器因无滚动区域无法把 input 滚到可见 → 系统判定 input 不可见而收键盘。手机号框（top≈180）在键盘上方可见所以正常；iQOO 老版浏览器自动 `resizes-content`（键盘弹出时布局视口收缩、RN ScrollView 高度随之变小、内容溢出可滚）所以正常。**修复**：`server/src/index.ts` 新增 `serveIndexHtml()`，读取/缓存 `public/index.html` 并把 viewport meta 替换为 `<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no, interactive-widget=resizes-content" />`，统一用于 `/`、`/index.html`（static 之前拦截中间件）与 SPA fallback（`*`）三处；强制新浏览器向 iQOO 老版行为对齐，键盘弹出时布局视口收缩 → RN ScrollView 获得滚动能力 → 密码框可滚到可见。肩带 `Cache-Control: no-store`。**验证**：playwright 移动 UA+390×460 视口下 ScrollView sh696>h460 可滚、`scrollIntoView` 后密码框 top208/bottom252 落于 460 内；`curl / `/`/register` 均返回注入后 meta。**前置修复仍保留为双保险**：`client/components/Screen.tsx` Web 端用普通 ScrollView 替代 KeyboardAwareScrollView、加 `focusin` 全局监听滚动到聚焦 input；register 密码框 `autoComplete="new-password"`（华为推荐规避密码保险箱值）。注意：`app/+html.tsx`/`web-static/index.html` 对 SPA export（`output='single'`）**不生效**（expo 用内置 `@expo/cli/static/template/index.html` 模板），必须走 server 端运行时注入；改后端仍须 `node build.js` + pkill 重启。另注意 `npx expo export` 覆盖 dist 时会**清掉手动补的 KaTeX 字体**（`dist/_expo/static/css/fonts/`），重新导出后需 `mkdir -p dist/_expo/static/css/fonts && cp node_modules/katex/dist/fonts/* dist/_expo/static/css/fonts/` 再同步到 server/public。
