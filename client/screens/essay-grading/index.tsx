...
2. 二段没写≠一定一档：若第一段情节完整、语言尚可（能力强但时间不够未完卷），最高可给到二档（6-10）；若第一段也只是零散碎片则一档。
...
【铁律 #1 —— 错误绝不降档（最高优先级）】语言表达（拼写/语法/词汇/错误数量与严重程度）绝对不允许作为降档依据，只能在同一档内微调分数（错误多→贴档内下沿），严禁因错误多/错得低级就把整篇压到更低档。判档唯一依据是情节/衔接质量。**只要情节完整且给出结局/升华（两段都算数），即使错误极其密集、极其基础，也锁死在三档（11-15）及以上，绝不允许因语言差降到二档**，只能在三档内贴下沿取分；语言能且只能把"情节完整"的作文在三档以上区间内微调，永远不能压到三档以下。只有情节本身不完整（缺结局但展开了一个冲突场景）或二段未写但第一段完整、语言尚可→二档；情节碎片/无推进/离题→一档或更低。评语与档位矛盾视为判错：只要评语承认"情节完整/有升华"，档位就不可能是二档或以下。
5. 只输出单一 total_score（不输出内容/语言/结构/书写子维度分），total_score 必须落在所判档区间内。
【铁律：提示句不算考生所写】题目给的第二段段首提示句（如 "Later that night, I received their apology messages."）不属于考生作文内容。凡第二段只有这句提示句、其后没有任何自主书写内容，一律按【二段未写】判定；不得因该句被 OCR 识别进文本而把第二段当作"有内容"。二段未写时：若第一段完整展开冲突场景且语言尚可→最高二档（通常6-8）；若第一段仍属情节碎片/几句话带过→一档（1-5）。`;
