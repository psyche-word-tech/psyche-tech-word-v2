...
【第一步：情节与融洽（基本资质）】要求续写必须写两段、总字数约150词（每段约80词），情节完整且与所给短文融合度高、与所提供的各段开头语衔接合理。

【情况一：任务完成度不达标而直接锁档】若续写总字数少于80词，或只写了一段，则直接归类为 1档 或 2档，不看情节融合（根本没资格谈融合）。在此基础上再用第二个指标细分：
- 若使用的语法结构和词汇较为丰富、准确 → 2档（6-10分）
- 若使用的语法结构和词汇错误多 → 1档（0-5分）

【情况二：任务完成度达标才进入 3/4/5 档】仅当写满了可辨认的两段、字数达到要求、情节完整且与原文融合度高、与各段开头语衔接合理时，才进入 3档/4档/5档（11-25），按下列区分：
- 5档（21-25）：与各段开头语衔接合理、续写两段内部衔接顺畅、逻辑严密、词汇语法多样化且复杂化程度高，其中语法与词汇拼写错误不超过 3-4 个。
- 4档（16-20）：在 5 档基础上，若逻辑不很严密、错误略多（约 8-12 个），但语法现象与词汇仍多样化。
- 3档（11-15）：衔接不太合理、逻辑不太严密、词汇与语法错误很多（12 个以上）、句间不太连贯、用词多为简单词。

【判分执行】先按上述两步锁定档位区间。档内给分一律【以所锁定档位的上限为起点，按错误程度向下扣分】：错误越少、越轻微，越接近档位上沿（最高即档位区间上限）；错误越多、越基础严重，越接近档位下沿。总原则：先取档位上沿，再按语法/拼写/用词/搭配合计的明显错误数量及严重度，从上沿起下扣（通常每个明显错误或每组同类错误扣 1-3 分），扣到贴近档位下沿为止，绝不在档位下沿之上凭空抬分。任务完成度严重缺失（缺段/缺字/情节残破）的续写，即使命中第二指标较好的情形，总分也只能落在第一档区间上的低端（约 1-3 分），绝不能因为判入第一档就默认取 5 分这类档位上限。只输出 total_score（不输出内容/语言/结构/书写等子维度分），total_score 必须落在锁定档的区间内。任务完成度不达标的续写，无论语言多好都绝不进入 3档及以上。评语中须写明：分档依据（第一步走到了哪种情况、字数/段落实况、语法词汇丰富度、错误数量）、以及档内如何从上沿扣分到该分数的理由（哪些错误各扣了多少）。`;
