...
export default function EssayGradingScreen() {
  ...
  const CONTINUATION_STANDARD = `英语读后续写评分（满分25，分六档：五档21-25、四档16-20、三档11-15、二档6-10、一档1-5、0分）。整体思路：拿文章【依次与五档、四档、三档、二档、一档的标准进行比对】。判定顺序必须从高到低：先比符合五档与否，分数进不了五档区间，再依序比四、三、二、一档；某一档的全部条件【同时满足】才算进入该档；否则继续下移比低一档。禁止跳档套分。

【五档（21-25）】同时符合以下所有要求才进五档：
1. 字数 140 词或以上，且这些字数与题目所给文章情节高度相关；
2. 运用了多样化的词汇（较多使用高中阶段才学习的较难词汇）；
3. 运用了多样化的语法现象；
4. 续写段落与原文提示段落衔接过渡非常顺畅，段落内部句子之间逻辑关系严密（多种衔接：因果关系、让步关系等）；
5. 语法错误或词汇拼写错误极少。
给分：以 0.5 为步长，在 21-25 内视 1-5 中各项的达成情况而定。

【四档（16-20）】字数 131 词或以上（含 131-139），情节高度相关（同五档第1条的"高度相关"）为基准，且同时：
1. 运用多样化的词汇，但其中高中阶段学的词汇占比不大；
2. 运用了多样化的语法现象；
3. 与原文提示段落衔接过渡一般，段落内部句子之间逻辑关系不太严密、逻辑连接词较少；
4. 语法错误或词汇拼写错误比较多（若按数计，可参考超过 10 处）。
给分：以 0.5 为步长，在 16-20 内视 1-4 达成情况而定。注意：131-139 词属四档资质；140+ 但未达五档其他要求时同样落入四档区间。

【三档（11-15）】字数 100-130 词、且两段都有内容、情节高度相关为基准，同时：
1. 运用的词汇有限，且其中高中阶段学的词汇占比很小；
2. 运用的语法现象有限；
3. 与原文提示段落衔接过渡一般，段落内部句子之间逻辑关系不严密、逻辑连接词很少；
4. 语法错误或词汇拼写错误很多（若按数计，可参考超过 15 处）。
给分：以 0.5 为步长，在 11-15 内视 1-4 达成情况而定。

【二档（6-10）】字数大于50且小于100词、两段都有内容、情节高度相关为基准，同时：
1. 运用的词汇有限，且基本为义务教育阶段词汇；
2. 运用的语法现象有限；
3. 与原文提示段落衔接过渡一般，段落内部句子之间逻辑关系不严密、逻辑连接词很少；
4. 语法错误或词汇拼写错误很多（若按数计，可参考超过 20 处）。
给分：以 0.5 为步长，在 6-10 内视 1-4 达成情况而定。

【一档（1-5）】字数小于 50 词、所写内容与所给文章情节有一定相关性为基准，同时：
1. 运用的词汇有限，且基本为义务教育阶段词汇；
2. 运用的语法现象有限；
3. 与原文提示段落衔接过渡一般，段落内部句子之间逻辑关系不严密、基本没有表示句间关系的连接词；
4. 语法错误或词汇拼写错误很多。
给分：以 0.5 为步长，在 1-5 内视 1-4 达成情况而定。

【0分】所写内容与所给文章完全无关，或没有写任何内容（原文给的提示句不算考生所写）。

【硬性贯穿规则】
1. 判定严格从最高档开始依序下移，先比高档次、进不了再比低档次，禁止直接跳档或先定低档再看能否上升，也禁止先定高档再因小错下调——档位完全由逐档比对的命中结果决定。
2. 只写了一段（缺第二段）时，晋级上限自动锁到二档——即最高只能落在二档区间内（因缺"两段都有内容"这一硬资质），无论其他方面多好都不得进入三四五档；且在二档内仍按错误数量、词汇语法达成度给分。
3. 错误数量按定性处理（极少 < 比较多 < 很多），不必逐个数清，用于档内定分与跨档辅助判断。
4. 档内给分一律以 0.5 为步长，落在该档区间内，越接近档位上限说明该档各项达成越好，越接近下沿说明达成欠缺越多。绝不在档外给分。
5. 只输出单一 total_score（不输出内容/语言/结构/书写等子维度分），total_score 必须落在所判档的区间内。
6. 评语须写明：逐档比对的过程（从五档起，每档因哪条未满足而落入下一档，最终落入哪一档），以及档内在该档区间内落到此分数的理由（该档各项达成情况、错误多少）。`;
